...
Sprite::Animation::Animation(const Animation &) = default;

Sprite::Animation::Animation(uint16_t foff, uint8_t num)
    : Animation(foff, num, 0.0f)
{
}
...
Sprite::Sprite(uint16_t sprN, int16_t rem, GraphicsBase::SpriteNumbers::SpriteTypes sprT, SpriteManager &spriteManager)
    : sprNum { sprN }
    , remap { rem }
    , sprType { sprT }
...
void Sprite::switchToAnim(uint32_t newId)
{
    ...
    anim = spriteManager.getAnimationById(newId);
    anim.set(Util::Animation::PLAY_FORWARD, Util::Animation::LOOP);
    animId = newId;
}

Pedestrian::Pedestrian(const Vector3D &e, const Vector3D &p, uint32_t id, SpriteManager &spriteManager, int16_t remapId)
    : GameObject_common(p)
    , Sprite(0, remapId, GraphicsBase::SpriteNumbers::PED, spriteManager)
...
void Pedestrian::update(uint32_t ticks)
{
    ...
    // FIXME: What's the point of this if statement if activeWeapon is set later unconditionally?
#if 0
...
#endif
    activeWeapon = chooseWeapon;
    auto checkAndSwitch = [this](uint32_t animId_, bool backwards = false) {
        if (animId_ != animId)
            switchToAnim(animId_);
        if (backwards)
            anim.set(Util::Animation::PLAY_BACKWARD, Util::Animation::LOOP);
    };
    switch (m_control.getMove()) {
        case 1:
            checkAndSwitch((int) m_control.getRunning() + 2u + activeWeapon * 3);
            break;
        case 0:
            checkAndSwitch(1u + activeWeapon * 3);
            break;
        case -1:
            checkAndSwitch(2u + activeWeapon * 3, true);
    }
    anim.update(ticks);
    ...
    //    XXX: Why the empty if statement was here?
    //    if (pedId == 0xffffffff) {
    //    }
    ...
    if (m_control.getFireWeapon() && ticks - lastWeaponTick > 400) {
        ...
        spriteManager.createProjectile(0, rot, pos, d1, ticks, pedId);
...
void Pedestrian::tryMove(Vector3D nPos)
{
    ...
    for (auto &[id, car] : spriteManager.getCars()) {
...
void Pedestrian::getShot(
    [[maybe_unused]] uint32_t shooterId, [[maybe_unused]] uint32_t dmg, [[maybe_unused]] bool front
)
{
    isDead = 1;
    switchToAnim(45);
...
CarSprite::CarSprite() = default;
CarSprite::CarSprite(const CarSprite &o) = default;
...
{
    // siren anim indices
    constexpr uint8_t dsi1 = 15;
    constexpr uint8_t dsi2 = 16;
    ...
    if (animState.get_item(10)) {
        if (ticks - lt_siren > 500) {
            if (deltaSet.get_item(dsi1)) {
                deltaSet.set_item(dsi1, false);
                deltaSet.set_item(dsi2, true);
            } else if (deltaSet.get_item(dsi2)) {
                deltaSet.set_item(dsi2, false);
                deltaSet.set_item(dsi1, true);
            } else {
                deltaSet.set_item(dsi1, true);
                deltaSet.set_item(dsi2, false);
            }
...
void Car::damageAt(const Vector3D &hit, uint32_t dmg)
{
     ...
     * 45    90
     */
    auto k = [](float angle) -> uint8_t {
        constexpr auto a1 = 60.0f;
        constexpr auto a2 = 180.0f - a1;
        constexpr auto a3 = 180.0f + a1;
        constexpr auto a4 = 360.0f - a1;
        // front left
        if (angle <= a1)
            return 3;
        // left side
...
SpriteObject::SpriteObject(OpenGTA::Map::ObjectPosition &op, uint32_t id, SpriteManager &spriteManager)
    : GameObject_common(Vector3D(INT2FLOAT_WRLD(op.x), 6.05f - INT2FLOAT_WRLD(op.z), INT2FLOAT_WRLD(op.y)))
    , Sprite(0, -1, GraphicsBase::SpriteNumbers::OBJECT, spriteManager)
    , OBox()
{
    objId = id;
    GraphicsBase &style = ActiveStyle::Instance().get();
...
SpriteObject::SpriteObject(
    const Vector3D &pos, uint16_t sprNum, GraphicsBase::SpriteNumbers::SpriteTypes sprT, SpriteManager &spriteManager
)
    : GameObject_common(pos)
    , Sprite(sprNum, -1, sprT, spriteManager)
    , OBox()
{
    isActive = true;
    m_M = TranslateMatrix3D(pos);
...
SpriteObject::SpriteObject(const SpriteObject &o)
    : GameObject_common(o)
    , Sprite(o)
    , OBox(o)
    , objId(o.objId)
