    ...
    , physfs_context_ { progname.data() }
{
    ...
    if ((arg_screen_h && !arg_screen_w) || (!arg_screen_h && arg_screen_w)) {
        WARN("Invalid screen specified: {}x{} - using default", arg_screen_w, arg_screen_h);
        arg_screen_h = 640;
        arg_screen_w = 480;
    }
...
void OpenGTAViewer::run()
{
    ...
    while (!global_Done) {
        ...
        OpenGTA::SpriteManager::Instance().creationArea.setRects(city->getActiveRect(), city->getOnScreenRect());
