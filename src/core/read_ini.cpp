...
void ScriptParser::loadLevel(UInt32 level)
{
    ...
    const auto end_of_section = sectionEndOffset(i->second);
    pf.seek(i->second);
    ...
    while (pf.tell() < end_of_section) {
        ...
        pf.read(buffer + offset, read_bytes);
        ...
        if (pf.tell() + read_bytes > end_of_section)
            read_bytes = end_of_section - pf.tell();
