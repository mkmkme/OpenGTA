...
bool CityView::getDrawTextured() const
{
    return drawTextured;
}
bool CityView::getDrawLines() const
{
    return drawLines;
}
bool CityView::getDrawLinesBlockColor() const
{
    return drawLinesBlockType;
}
...
void CityView::setDrawLinesBlockColor(bool v)
{
    drawLinesBlockType = v;
}
...
#define MSWAP(a, b)                \
    {                              \
        GLfloat tmp = sideTex1[a]; \
        sideTex1[a] = sideTex1[b]; \
        sideTex1[b] = tmp;         \
    }
...
#define GLTEX_HELPER                              \
    do {                                          \
        glTexCoord2f(lidTex[jj], lidTex[jj + 1]); \
        jj += 2;                                  \
        if (jj > 6) {                             \
            jj = 0;                               \
        }                                         \
...
#define GLTEX1_HELPER                                 \
    do {                                              \
        glTexCoord2f(sideTex1[jj], sideTex1[jj + 1]); \
        jj += 2;                                      \
        if (jj > 6) {                                 \
            jj = 0;                                   \
        }                                             \
    ...
    if (bi->right) {
        ...
        if (!banim.has_value()) {
            if (!sideCache->hasTexture(bi->right)) {
                right_tex = ImageUtil::createGLTexture(
                    64,
                    64,
                    is_flat,
    ...
    if (bi->top) {
        ...
        if (!banim.has_value()) {
            ...
            if (!sideCache->hasTexture(bi->top)) {
                top_tex = ImageUtil::createGLTexture(
                    64,
                    64,
                    is_flat,
    ...
    if (bi->bottom) {
        ...
        if (!banim.has_value()) {
            ...
            if (!sideCache->hasTexture(bi->bottom)) {
                bottom_tex = ImageUtil::createGLTexture(
                    64,
                    64,
                    is_flat,
...
#define MSWAP(a, b)                       \
    do {                                  \
        sideTex1[a] = 1.0f - sideTex1[a]; \
        sideTex1[b] = 1.0f - sideTex1[b]; \
    } while (false)
            ...
            if (bi->left) {
                ...
                if (drawTextured) {
                    ...
                    for (int j = 0; j < 4; j++) {
                        ...
                        scene_rendered_vertices += 1;
                        glVertex3f(
                            SLOPE_RAW_DATA[which][4][j][0] - 1.001f, // -1.001f
                ...
                if (drawLines) {
                    ...
                    for (int j = 0; j < 4; j++) {
                        glVertex3f(
                            SLOPE_RAW_DATA[which][4][j][0] - 1.001f,
                            SLOPE_RAW_DATA[which][4][j][1],
                            SLOPE_RAW_DATA[which][4][j][2]
                        );
                    }
                    glVertex3f(
                        SLOPE_RAW_DATA[which][4][0][0] - 1.001f,
